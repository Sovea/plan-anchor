...
- **State file** — one Markdown file per task at `.claude/plan-anchor/<slug>.md`, with the currently attached slug tracked in `.claude/plan-anchor/current.txt`. This file is the single source of truth for mission, plan, Work Units, verification, drift, and handoff. See `state/template.md`.
...
- **Commands** — `/plan-anchor:start`, `/plan-anchor:status`, `/plan-anchor:drift`, `/plan-anchor:handoff`, `/plan-anchor:resume`, `/plan-anchor:next`, `/plan-anchor:done`, `/plan-anchor:revise`. These are the primary UI; prose triggers are a fallback only. Definitions live in `commands/*.md`. `/plan-anchor:start <description>` creates a new task and may switch away from an unfinished current task without requiring `/plan-anchor:done`; `/plan-anchor:resume <slug>` switches to an existing task and runs recovery; `/plan-anchor:revise [<slug>]` layers a revision onto any task (defaults to current) — new ACs/WUs appended via plan mode; `complete` re-opens to `active`, `active`/`blocked` keep their current WU.
