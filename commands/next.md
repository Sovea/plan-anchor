...
allowed-tools: [Read, Bash, Skill, Edit, Write, MultiEdit, Grep, Glob, TaskList, TaskUpdate]
---
...
| 5 | No active Work Unit but at least one is `pending` | **auto-promote** the first pending WU to `active`: edit `.claude/plan-anchor/<slug>.md` to set its `## WU-N` heading status to `active`, update frontmatter `active_wu`, bump `updated`, then update the mirrored native Task to `in_progress`. Print `[next] Promoted <WU-id> to active (rule 5) → continuing implementation.` Then fall through to rule 6 behavior: read the WU's first unchecked `Done when` and begin work. | No reason to stall the user when the next WU is unambiguous. |
...
For rules 5 and 6 (continue implementation): do not call `Skill`. Continue the turn directly. For rule 5, first promote the next pending WU (edit the state file + update the native Task), then proceed as rule 6: read the WU's first unchecked `Done when` and begin implementation (Read / Grep / Edit / Write / Bash as needed).
...
- At most one sub-command dispatch per `:next` call (rules 1–4, 7). Rules 5 and 6 do not dispatch; they continue implementation in the same turn until the agent naturally yields. If the user wants to chain another `:next` after that, they re-run it.
