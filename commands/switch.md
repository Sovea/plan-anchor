...
Run /plan-anchor:resume to align with the repo and see the next action.
```

Do **not** run the full resume sequence here. `/plan-anchor:switch` is meant to be cheap and side-effect-free beyond the pointer update; the next `/plan-anchor:resume`, `/plan-anchor:status`, or `SessionStart` hook will pick it up.
